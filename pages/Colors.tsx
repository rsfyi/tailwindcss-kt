const colorList = [ 'gray', 'red', 'orange', 'yellow', 'green', 'teal', 'indigo', 'blue', 'purple', 'pink' ];
		...
		<div className='flex  mt-8 flex-col'>
			...
			<div className='flex justify-between mt-4'>
				{colorList.map((color, i) => (
					<div key={i}>
						<h1 className='text-xl font-semibold mb-4'>{color.toLowerCase()}</h1>
						{shades.map((shade, j) => (
							<div key={j} className={`w-16 h-16 bg-${color}-${shade} flex justify-center items-center`}>
								{shade}
							</div>
						))}
					</div>
